...
// added by yew
static int mainargc;
static char *const *mainargv;
static int (*mainpfn)(int, char*const*);
static SERVICE_STATUS_HANDLE ngx_sstatushandle;
static SERVICE_STATUS ngx_sstatus;
...
servicehandler(DWORD controlCode)
{
    ...
    case SERVICE_CONTROL_STOP:
      ngx_sstatus.dwCurrentState = SERVICE_STOP_PENDING;
      SetServiceStatus(ngx_sstatushandle,&ngx_sstatus);
      ngx_console_handler(CTRL_CLOSE_EVENT);
      break;
    }
}
...
int ngx_service(int (*pfn)(int, char*const*), int argc, char *const *argv)
{
    ...
    ngx_sstatushandle = NULL;
    ZeroMemory(&ngx_sstatus, sizeof(ngx_sstatus));
    ZeroMemory(&serviceName, sizeof(serviceName));

    ngx_sstatus.dwServiceType = SERVICE_WIN32;
    ngx_sstatus.dwCurrentState = SERVICE_START_PENDING;
    ngx_sstatus.dwControlsAccepted = SERVICE_ACCEPT_STOP;
    ngx_sstatus.dwWin32ExitCode = NO_ERROR;
    ngx_sstatus.dwServiceSpecificExitCode = NO_ERROR;
    ngx_sstatus.dwCheckPoint = 0;
    ...
    // in service mode, but failed with other reasons
    return 1;
}