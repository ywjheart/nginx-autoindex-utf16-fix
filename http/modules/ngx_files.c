...
ngx_open_dir(ngx_str_t *name, ngx_dir_t *dir)
{
    ...
    dir->namelen = 0; // clear cached utf-8 buffer
...
static u_char *
ngx_utf8_encode_char(u_long ch, u_char *str) {
    if (ch <= 0x7f) {
        *str++ = (u_char)ch;
    } else if (ch <= 0x0007ff) {
        *str++ = (u_char)(0xc0 | (ch >>  6) & 0x1f);
        *str++ = (u_char)(0x80 | (ch & 0x3f));
    } else if (ch <= 0x00ffff) {
        *str++ = (u_char)(0xe0 | (ch >> 12) & 0x0f);
        *str++ = (u_char)(0x80 | (ch >>  6) & 0x3f);
        *str++ = (u_char)(0x80 | (ch & 0x3f));
    } else if (ch <= 0x10ffff) {
        *str++ = (u_char)(0xf0 | (ch >> 18) & 0x07);
        *str++ = (u_char)(0x80 | (ch >> 12) & 0x3f);
        *str++ = (u_char)(0x80 | (ch >>  6) & 0x3f);
        *str++ = (u_char)(0x80 | (ch & 0x3f));
    }
    return str;
...
static u_char *
ngx_utf16_to_utf8(u_char *utf8, u_short *utf16, size_t *len)
{
    u_short   *u;
    u_char    *p;
    u_long    surrogate;
    ...
    for ( ;; ) {
        ...
        if ((*u >= 0xd800) && (*u <= 0xdbff)) {
            surrogate = (*u - 0xd800) << 10;
            if (!((*(u+1) >= 0xdc00) && (*(u+1) <= 0xdfff))) {
                // Broken surrogate pair, out as is
                p = ngx_utf8_encode_char(*u, p);
            }
        } else if ((*u >= 0xdc00) && (*u <= 0xdfff)) {
            surrogate |= *u - 0xdc00;
            p = ngx_utf8_encode_char(surrogate + 0x10000, p);
        } else {
            p = ngx_utf8_encode_char(*u, p);
        }
...
ngx_de_name(ngx_dir_t *dir)
{
    ...
    if (!dir->namelen) {
        len = NGX_UTF8_BUFLEN;
        u = ngx_utf16_to_utf8(dir->name, dir->finddata.cFileName, &len);
        dir->namelen = len;
    }
    return dir->name;
}
